...
def main():
    ...
    # Data is written as its own same-origin JS file (not inlined into the
    # HTML) because the page's CSP has no 'unsafe-inline' in script-src —
    # an inline <script> assigning window.WORKS_DATA gets silently blocked
    # by the browser, which used to crash ArtWorksPanel with a
    # ReferenceError. External same-origin scripts are allowed by 'self'.
    data_js = (
        f"window.WORKS_DATA = {json.dumps(data['works_data'], ensure_ascii=False)};\n"
        f"window.AD_PM_DESC_PLACEHOLDER = {json.dumps(data['extra'].get('AD_PM_DESC_PLACEHOLDER', ''), ensure_ascii=False)};\n"
        f"window.DISCIPLINE_BACKDROPS = {json.dumps(data['extra'].get('DISCIPLINE_BACKDROPS', {}), ensure_ascii=False)};\n"
        f"window.DISCIPLINE_LABELS = {json.dumps(data['extra'].get('DISCIPLINE_LABELS', {}), ensure_ascii=False)};\n"
    )
    with open(os.path.join(OUTPUT_DIR, "data.js"), "w", encoding="utf-8") as f:
        f.write(data_js)
    data_js_v = hashlib.md5(data_js.encode("utf-8")).hexdigest()[:8]
